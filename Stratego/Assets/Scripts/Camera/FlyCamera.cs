...
public class FlyCamera : MonoBehaviour
{
    ...
    private float mainSpeed = 80; //regular speed
    private float shiftAdd = 100; //multiplied by how long shift is held.  Basically running
    private float maxShift = 120; //Maximum speed when holdin gshift
    private float scrollSpeed = 20; //Maximum speed when holdin gshift
    ...
    void Update()
    {
        ...
        if (transform.position.y < 3) // zorgt scrolling dat je onder de 3 komt op de y as? terugzetten oude pos (te dicht bij grond)
        {
            transform.position = positionBeforeChange;
        }
        else if (transform.position.y > 100) // zorgt scrolling dat je boven de 100 komt op de y as? terugzetten oude pos (te ver vd grond)
        {
            transform.position = positionBeforeChange;
        }
